...
1. **提出順と可否**: issue（BUGREPORT）→ PR 2（SMTP 1行）→ PR 3（loadstring）→ PR 1
   （BackgroundTask、いちばん重い）の順を提案。どれをいつ出すか。
2. **x64 ビルドをやるか**: PR 1 の実機検証 + FMP19 での idle-SQL 再現確認の両方に効く。
   WORK1 に FMP19 あり。vcxproj は x64 構成を持っている（上流の標準構成）ので、
   Libraries/win64 の取得（上流は git LFS）から。
3. PR 提出時は veltrea アカウントの fork（origin）から upstream へ。PR 本文の英語ドラフトは
   各ブランチのコミットメッセージがそのまま使える（コミットメッセージ = PR 本文になる想定で書いてある）。
